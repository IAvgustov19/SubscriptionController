...
public class Handler
{
    ...
    public async Task<Response> FunctionHandler(Request request, Context context)
    {
        ...
        UPSERT INTO accounts (ID, ExpiredDate, Subscription) VALUES ($id, null, false)";
        ...
        SELECT Email FROM accounts VIEW ExpiredDateIndex
        WHERE ExpiredDate == $date AND Subscription == true";
            ...
            return await session.ExecuteDataQuery(
                ...
                parameters: new Dictionary<string, YdbValue>
                {
                    { "$date", YdbValue.MakeDate(DateTime.Now.AddDays(3)) },
                }
            );
        });
        response.Status.EnsureSuccess();
        ...
        SendEmail(resultSet.Rows, ExpiredDateType.ThreeDay);
        ...
        WHERE ExpiredDate == $date AND Subscription == true";
            ...
            return await session.ExecuteDataQuery(
                ...
                parameters: new Dictionary<string, YdbValue>
                {
                    { "$date", YdbValue.MakeDate(DateTime.Now.AddDays(1)) },
                }
            );
        });
        response.Status.EnsureSuccess();
